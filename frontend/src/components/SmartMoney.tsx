...
function Positions({ t }: { t: Trader }) {
  if (t.positions === null) {
    return (
      <div className="text-[11px] text-slate-500">{t.positions_note || "positions unavailable"}</div>
    );
  }
  if (t.positions.length === 0) {
    return <div className="text-[11px] italic text-slate-500">no current World Cup position</div>;
  }
  return (
      ...
      {t.positions.map((p, i) => (
        <li key={i} className="rounded-md bg-ink-800/60 px-2 py-1 text-[11px]">
          <div className="flex items-center justify-between gap-2">
            ...
            {p.outcome && (
              <span className="chip shrink-0 bg-accent/10 text-accent">{p.outcome}</span>
            )}
          </div>
          <div className="numeric mt-0.5 flex flex-wrap gap-x-3 text-[10.5px] text-slate-500">
  ...
  return (
    <div className="card overflow-hidden">
      ...
      <div className="p-4">
        ...
        {!data.available ? (
          <div className="rounded-lg border border-white/[0.05] bg-ink-800/50 px-3 py-3 text-[12px] text-slate-500">
          ...
          <div className="space-y-2.5">
            ...
            {data.leaderboard.map((t) => (
              <div
                key={t.address}
                className="border-t border-white/[0.05] pt-2.5 first:border-0 first:pt-0"
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="flex min-w-0 items-center gap-2">
                    <span className="numeric w-5 shrink-0 text-[11px] text-slate-500">
                      {t.rank ?? DASH}
                    </span>
